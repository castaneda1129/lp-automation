...
async function requestResearch(data) {
  const businessType = data.businessType || data.industry || 'ビジネス';
...
簡潔なMarkdown（全体で600〜800文字程度）。LP制作者がすぐ使えるよう具体的に。`;
  ...
  try {
    const result = execSync(
      `/usr/bin/ssh -o ConnectTimeout=10 -o StrictHostKeyChecking=no kaichi@100.65.2.126 "/opt/homebrew/bin/node /opt/homebrew/bin/openclaw agent --agent nakamura --local -m '${prompt.replace(/'/g, "'\\''")}' 2>&1"`,
      {
        timeout: 120000, // 2分
