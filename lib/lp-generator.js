...
function buildDetailsSection(data) {
  ...
  // ヒーロー
  if (data.hero_copy) lines.push(`\n## ヒーロー・キャッチコピー（一言一句このまま使うこと）\n${data.hero_copy}`);
  if (data.hero_image) lines.push(`- 背景写真イメージ: ${data.hero_image}`);
  if (data.cta_text) lines.push(`- CTAボタン文言: ${data.cta_text}`);

  // 実績
  if (data.achievements || data.before_after) {
    lines.push(`\n## 実績・数字（一言一句このまま使うこと）`);
    if (data.achievements) lines.push(data.achievements);
    if (data.before_after) lines.push(`ビフォーアフター事例:\n${data.before_after}`);
  }

  // お客様の声（動的に複数取得）
  const voices = [];
  for (let i = 1; i <= 10; i++) {
    const name = data[`voice_name_${i}`];
    const text = data[`voice_text_${i}`];
    if (name || text) voices.push({ name: name || '匿名', text: text || '' });
  }
  if (voices.length > 0) {
    lines.push(`\n## お客様の声（以下を一言一句そのまま使うこと）`);
    voices.forEach((v, i) => {
  ...
  const plans = [];
  for (let i = 1; i <= 10; i++) {
    const name = data[`plan_name_${i}`];
    const price = data[`plan_price_${i}`];
    const detail = data[`plan_detail_${i}`];
    if (name || price) plans.push({ name: name || '', price: price || '', detail: detail || '' });
  }
  if (plans.length > 0) {
    lines.push(`\n## 料金・プラン（以下を一言一句そのまま使うこと）`);
    plans.forEach((p, i) => {
      lines.push(`プラン${i + 1}: ${p.name} / ${p.price}\n内容: ${p.detail}`);
    });
    if (data.price_note) lines.push(`補足: ${data.price_note}`);
  }

  // FAQ（動的に複数取得）
  const faqs = [];
  for (let i = 1; i <= 10; i++) {
    const q = data[`faq_q_${i}`];
    const a = data[`faq_a_${i}`];
    if (q || a) faqs.push({ q: q || '', a: a || '' });
  }
  if (faqs.length > 0) {
    lines.push(`\n## よくある質問（以下を一言一句そのまま使うこと）`);
    faqs.forEach((f, i) => {
  ...
  // その他
  if (data.staff_profile) lines.push(`\n## スタッフ・担当者プロフィール（一言一句そのまま使うこと）\n${data.staff_profile}`);
  if (data.location) lines.push(`\n## アクセス・場所\n${data.location}`);
  if (data.other) lines.push(`\n## その他クライアントからの要望（必ず反映すること）\n${data.other}`);

  if (lines.length === 0) return '';

  return `\n⚠️ 以下のクライアント提供情報は**一言一句そのまま**LPに使用すること。要約・改変・省略は絶対禁止。\n${lines.join('\n')}`;
}
...
## クライアント情報
- 会社名/サービス名: ${data.companyName || data.businessName || data.serviceName}
- 業種: ${data.businessType || data.industry || '未入力'}
- ターゲット顧客: ${data.targetAudience || data.target}
- 主な訴求ポイント: ${data.mainMessage || data.appeal || data.appealPoints || '未入力'}
- 強み・特徴: ${data.strengths || '未入力'}
- エリア: ${data.location || '未入力'}
- CTA（行動喚起）: ${data.cta_text || data.cta || '無料相談はこちら'}
- 担当者メール: ${data.contact || data.contactEmail || data.email}
${detailsSection}
